...
def ConvertCsv(csvPath:str, targetCity:str):
    ...
    SaveToJson(dataForCity, f"./jsons/{targetCity}_temperatures.json")

def SaveToJson(obj, pathToSave):
    with open(pathToSave, "w") as jsonFile:
        json.dump(obj, jsonFile, indent=2)


ConvertCsv("./temperature.csv", "Seattle")  # c=celcius f=fahrenheit